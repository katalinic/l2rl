...
class MetaRLNetwork(object):
    def __init__(self, task, number_of_actions, hidden_units = 48, learning_rate = 1e-4, beta_v = 0.05, beta_e = 0.05, number_of_episodes = 2e4):
        self.task = task
    ...
    def _build_placeholders(self):
        self.prev_r_pl = tf.placeholder(tf.float32, [None, self.number_of_actions+1]) if self.task==3 else tf.placeholder(tf.float32, [None,1])
    ...
    def _build_network(self):
        ...
        unrolled, self.rnn_state = tf.nn.dynamic_rnn(cell, self.input_pl, initial_state = self.init_rnn_state, dtype=tf.float32)
        unrolled = tf.reshape(unrolled, [-1, self.hidden_units])
        self.logits = tf.contrib.layers.fully_connected(unrolled, self.number_of_actions, activation_fn = None)
    ...
    def _build_optimisation(self):
        ...
        logp = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits,labels=self.target_a_pl)
